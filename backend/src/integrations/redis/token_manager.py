...
Migrated from voxy_agents/core/auth_token_manager.py to integrations/redis/.
"""
...
class TokenManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.bind(event="AUTH_TOKEN|REDIS_CONNECT").info(
                "Redis connected for token management"
            )
        except Exception as e:
            logger.bind(event="AUTH_TOKEN|REDIS_CONNECT_ERROR").warning(
                "Redis connection failed - token blacklisting disabled", error=str(e)
            )
    ...
    async def blacklist_token(
        self, jti: str, user_id: str, expiration: datetime
    ) -> bool:
        """
        ...
        if not self.redis_client:
            logger.bind(event="AUTH_TOKEN|BLACKLIST_UNAVAILABLE").warning(
                "Redis not available, cannot blacklist token"
            )
            return False
        ...
        try:
            ...
            if ttl <= 0:
                logger.bind(event="AUTH_TOKEN|BLACKLIST_EXPIRED").debug(
                    "Token already expired, no need to blacklist", jti=jti[:16]
                )
                return True
            ...
            self.redis_client.setex(blacklist_key, ttl, json.dumps(token_info))

            logger.bind(event="AUTH_TOKEN|BLACKLIST_SUCCESS").info(
                "Token blacklisted", jti=jti[:16], user_id=user_id[:16], ttl_seconds=ttl
            )
            return True

        except Exception as e:
            logger.bind(event="AUTH_TOKEN|BLACKLIST_ERROR").error(
                "Failed to blacklist token", jti=jti[:16], error=str(e)
            )
            return False
    ...
    async def is_token_blacklisted(self, jti: str) -> bool:
        ...
        try:
            ...
            if is_blacklisted:
                logger.bind(event="AUTH_TOKEN|IS_BLACKLISTED").debug(
                    "Token is blacklisted", jti=jti[:16]
                )
            ...
            logger.bind(event="AUTH_TOKEN|CHECK_BLACKLIST_ERROR").error(
                "Failed to check token blacklist", jti=jti[:16], error=str(e)
            )
            # On error, assume token is valid for security
            return False

    async def store_token_info(
        self, jti: str, user_id: str, email: str, expiration: datetime
    ) -> bool:
        """
        ...
        try:
            ...
            self.redis_client.setex(info_key, ttl, json.dumps(token_info))

            return True

        except Exception as e:
            logger.bind(event="AUTH_TOKEN|STORE_INFO_ERROR").error(
                "Failed to store token info", jti=jti[:16], error=str(e)
            )
            return False
    ...
    async def get_token_info(self, jti: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.bind(event="AUTH_TOKEN|GET_INFO_ERROR").error(
                "Failed to get token info", jti=jti[:16], error=str(e)
            )
            return None
            ...
            for info_key in info_keys:
                try:
                    ...
                    logger.bind(event="AUTH_TOKEN|PROCESS_TOKEN_ERROR").error(
                        "Error processing token", info_key=info_key, error=str(e)
                    )
                    continue

            logger.bind(event="AUTH_TOKEN|BLACKLIST_USER_TOKENS").info(
                "Blacklisted user tokens", user_id=user_id[:16], count=blacklisted_count
            )
            return blacklisted_count

        except Exception as e:
            logger.bind(event="AUTH_TOKEN|BLACKLIST_USER_TOKENS_ERROR").error(
                "Failed to blacklist user tokens", user_id=user_id[:16], error=str(e)
            )
            return 0
